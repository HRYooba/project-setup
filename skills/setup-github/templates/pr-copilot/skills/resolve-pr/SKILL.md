...
description: >
  ...
  「PRレビュー解決」「レビュー自動対応」「CI直して」「/resolve-pr」と依頼した場合に使用される。
  PRレビューコメントと失敗した CI チェックの取得・修正実装・コミット・Push・
  リプライ送信・Copilotコメントの自動Resolveまでを一括自動実行する。
  PR番号またはURLを指定。
version: 1.1.0
...
CI 失敗だけを直した場合は subject をその内容に合わせる（例: `fix: コーディング規約違反を解消`）。
