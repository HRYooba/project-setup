...
7. **Copilot 自動アサイン** — `gh pr create` 直後、コード変更を含む PR に Copilot レビュアーを自動で付ける（PostToolUse hook）
8. **watch-pr** — PR レビューをポーリング監視し、指摘を検出したら resolve-pr を自動起動（skill）
...
- `.claude/CLAUDE.md` はマーカー（`**ブランチ**:` / `**レビュー**:` / `**レビュー対応**:`）が既にあれば追記しない（冪等）。例外: レビュー行が旧テンプレ文面と完全一致する場合のみ `--required` 案内付きの新文面へ置換する（手編集された行は触らない）
