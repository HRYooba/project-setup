...
description: >
  ...
  ブランチ保護（.githooks/pre-push + SessionStart 自動有効化）、PR 前レビュー運用
  （/simplify + /security-review のソフト指示）、git-conventions ルール、
  create-issue skill を撒く。PR 自動レビュー3機能（Copilot 自動アサイン / watch-pr /
  resolve-pr）と AGENTS.md 自動生成（Copilot code review にプロジェクト規約を教える）の
...
6. **CLAUDE.md** — ブランチ規約（作業ブランチ経由の PR 必須）と PR 前レビュー運用「PR 作成前に `/simplify` を 1 回（再利用・簡素化・効率の整理）」「PR 作成前に `/security-review` を 1 回」を**ソフト指示**として追記（gate 休眠のため強制はしない）。旧テンプレの code-review 必須行は `/simplify` 行へ、旧 security ブロック行は soft 版へ、再実行時に完全一致置換で移行する（手編集された行は触らず警告）
...
- `.claude/CLAUDE.md` はマーカー（`**ブランチ**:` / `**簡素化**:` / `**セキュリティレビュー**:`）が既にあれば追記しない（冪等）。例外: 旧 code-review 行（`**レビュー**:`）は `/simplify` 行へ、旧 security ブロック行は soft 版へ、旧テンプレ文面と完全一致する場合のみ移行する（手編集された行は触らず警告する）。旧版が撒いた「## PR レビュー」節（`**レビュー対応**:`）は配布廃止につき、旧テンプレ文面と完全一致するときだけ再実行時に除去する（手編集された節は警告して残す）
