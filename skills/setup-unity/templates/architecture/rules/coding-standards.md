...
定数は専用のクラス（`Shared` 名前空間下、または該当 Bounded Context 直下）で管理すること

### 名前空間構造

- レイヤー直下は**Bounded Context 別**に分割する。
...
- 新しい Service / 実装 / DTO を追加する前に、同じ層・同じ種別の既存クラスを検索し、
  戻り値型・ライフサイクル・命名を揃える（種別ごとの基準は `rules/class-catalog.md`）

### 設計アプローチ

**最小変更で済ませない**。「現行の挙動を維持した最小変更」は既存の複雑さを温存し、
パッチの蓄積で密結合が増大する。根本解決のためにクラス設計・依存関係・所属レイヤーの
変更を躊躇しないこと:
...
- Spawner/Factory はその操作だけを担い、初期化ロジックは Presenter が行う
