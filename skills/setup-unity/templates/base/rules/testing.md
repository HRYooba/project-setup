...
- 純ロジック（バリデーション、計算、変換、境界条件）の変更
- 状態を持つクラスの状態遷移、外部購読される初期値、入力正規化、Clear 処理の変更
- 成功/失敗分岐、rollback / fallback、エラー変換、呼び出し順序、キャンセル伝播の変更
- DTO 変換、URL / path / header 組み立て、レスポンス解釈、エラーコード変換、認証情報の扱いの変更

以下の変更では、テスト不要と判断してよい。ただし完了報告に理由を記載する:
- MonoBehaviour・シーン・UI の見た目・配置・Inspector 配線のみの変更（ランタイム挙動はユーザー手動確認）
...
初期値テストは、外部から購読される状態（State / UI Model 等）の初期契約を守る場合に限る。
...
| 変更内容 | 実行するチェック |
|:---|:---|
| ロジックを含む `.cs` 変更 | `/test-unity`（テスト責任の判定 + 必要なテスト追加/更新 + 実行） |
| MonoBehaviour・シーン・UI に関わる `.cs` 変更 | コンパイル確認（`rules/unity-mcp.md`）→ ユーザー動作確認 |
