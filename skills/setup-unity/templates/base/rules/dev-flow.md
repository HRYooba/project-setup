...
| 変更内容 | 実行するチェック |
|:---|:---|
| **plain class** の `.cs` 変更（ロジックを含むもの） | `rules/testing.md` に従いテスト要否を判定し、必要なら追加/更新して差分のスコープで実行する |
| **MonoBehaviour / UXML / シーン依存**の `.cs` 変更 | コンパイル確認（`rules/unity-cli.md`）→ 下記「ランタイム動作確認」 |
