...
`--help` / `-h` / `--version` は常に通す。フラグや exit code を確かめる唯一の手段なので、
塞ぐと順番待ち中に何も調べられない。
...
- 他の worker の変更と統合したときに通る保証はない
- `Library/` を持ち回るので、クリーンな環境でのインポート結果と一致する保証はない

**統合後はレーンでもう一度検証する**（統合した commit を載せて回す）。CI は当てにできない —
配布する workflow は Editor を起こさず、プロジェクト整合性だけを見る。クリーンな
インポートまで確かめたいときは `Library/` を消してから載せる。
