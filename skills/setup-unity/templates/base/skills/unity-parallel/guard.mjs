...
//
// Unity Editor は 1 つしかなく、MCP の接続先選択はクライアント単位のグローバル状態なので、
// 「今 Editor を借りている 1 人」以外が Unity MCP を呼ぶと、成功応答のまま別のスナップショット
// を検証してしまう（＝偽の green）。それを機械的に止めるのがこのフックの役目。
//
...
//
// 何を止められないか（正直な限界。README にも同じことを書く）:
//   - シェル越しの書き込みはコマンド文字列のヒューリスティックでしか見られない
...
function main() {
  ...
  if (state.recovery) {
    if (toolName.startsWith("mcp__") || touchesLane(input, state.lanePath)) {
      deny(
  ...
  let prefixes = null;
  try {
    prefixes = unityMcpPrefixes(cwd);
  } catch {
    prefixes = null;
  }

  if (isUnityMcpTool(toolName, prefixes)) {
    if (!holder) {
      if (agentId) {
        deny(
          `Editor は誰にも貸し出されていません。サブエージェントは lane.mjs request で順番待ちに入ってください（tool: ${toolName}）。`
        );
      }
    ...
    if (isHolder || isDelegate) {
      if (!MCP_ALLOWED_PHASES.has(holder.phase)) {
        deny(
          `Editor の準備が終わっていません（phase: ${holder.phase}）。ACTIVE になるまで Unity MCP は呼べません。` +
            `準備中に呼ぶと、切り替え前のスナップショットを検証して green を出してしまいます。`
        );
      }
    ...
    if (agentId === null) {
      ...
      if (COORDINATOR_MCP_PHASES.has(holder.phase)) allow();
      deny(
    ...
    if (filePath && isUnitySerialized(filePath)) {
      deny(
        `${filePath} は Unity がシリアライズするファイルです。手編集は GUID / fileID を壊し、` +
          `しかも壊れたことがその場では分かりません。Editor（Unity MCP）経由で変更してください。`
      );
    }
  ...
  // --- 3. シェル ----------------------------------------------------------
  if (/^(Bash|PowerShell)$/.test(toolName)) {
    const command = toolCommand(input);

    // coordinator への正規の呼び出しは、下のレーン言及チェックより先に通す
    // （lane.mjs 自体がレーンの中にあるため、後段のチェックに引っかかってしまう）。
    // ここで呼び出し元の識別子を記録するのが、identity を機械的に決める唯一の手段。
    // エージェントは自分の agent_id を知らないので、自己申告させると identity が意味を失う。
    if (/lane\.mjs(["']?)\s/.test(command) || /lane\.mjs["']?$/.test(command.trim())) {
