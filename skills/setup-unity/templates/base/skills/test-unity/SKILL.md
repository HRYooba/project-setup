...
**`--filter` に何を渡すか（この節が正本）:**

**B1 で検出したテストの名前空間**を渡す（`*Test.cs` の `namespace` 宣言から取る。asmdef 名とは
一致しないので、ファイルの中身を見る）。名前空間が特定できなければ対象を絞れないので、
**実行せずその旨を報告して B6 へ進む**。

- **全件実行はしない。** 全件は `.github/workflows/unity-ci.yml` の test ジョブが PR ごとに回す。
  ここで全件を回すと同じ実行を 2 箇所で払う上、差分と無関係な失敗が混ざって切り分けが濁る
...
- `unity test` の exit code と、それが「テスト失敗」か「実行できなかった」かの区別。
  ここで回したのは**差分のスコープだけ**である旨も添える（全件の結果は PR の `unity-ci` が出す）
