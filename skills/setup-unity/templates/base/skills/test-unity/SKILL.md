...
| 2 | B2 | コンソールエラー取得 + **仕様ソース** + 全対象 + 依存型 + 既存テストを Read → **設計**（designing-guide でケース選別） |
...
**「コンソールエラー取得」（count=20）+ 仕様ソース + 全対象ファイル + 依存型 + 同名既存テストを 1 レスポンスにまとめて実行**（逐次読み禁止）。
...
報告項目は `rules/testing.md`「完了報告」が単一ソース。この skill 固有の追加項目は次の 3 つ:
- 対象ファイルと、参照した仕様ソース（無ければ「仕様ソース無し」と明記）
- 重複整理: 削除・統合した内容、削除を提案した既存テストとその根拠
- コンパイル確認の結果 / テスト実行の結果（既知失敗を除いた判定）
