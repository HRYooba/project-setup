...
// ---- 状態ファイル .setup-sync.json への setup-unity キーの記録 ----
...
if (pluginVersion) {
  ...
  console.log(
    "注意: .claude-plugin/plugin.json のバージョンを読めなかったため .setup-sync.json を書きませんでした（テンプレ自動追随は無効のまま）。"
  );
}
...
// 状態ファイル `.claude/.setup-sync.json` へ自分のキー（skillKey）をマージ更新する。相手のキーや
...
function writeSyncState(skillKey, version, flags) {
  ...
  if (existsSync(p)) {
    try {
      ...
      console.log("注意: .setup-sync.json が不正な JSON のため作り直します（他スキルのキーは失われる可能性あり）。");
    }
  }
...
if (syncState) console.log(`状態ファイル(.setup-sync.json): ${syncState}`);
