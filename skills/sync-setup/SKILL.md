...
「矛盾したときの扱い」に従い、テンプレ側を採って**apply が出力した「矛盾メモの書き出し先」**へ
矛盾の中身を書き残す（パスはリポジトリごとに違うので、出力に出ているものを使う）。publish が
...
- **フェーズを飛ばさない**。`--phase` 無しの実行はエラーになる（apply と publish の間の統合工程を
  スキップさせないため）。publish は apply が残した同期計画が無いとエラー終了する（置き場所は
  エラー文に出る。データディレクトリは全配備先で共有されるため、計画と矛盾メモは
  リポジトリごとのファイルに分けてある — 別リポジトリの同時実行に踏まれない）
