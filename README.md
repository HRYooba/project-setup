...
| skill | 内容 |
|:------|:-----|
| `setup-github` | GitHub 開発フロー一式を導入する。ブランチ保護（pre-push。既定 ON・質問で外せる）、PR 前レビュー運用（`/simplify` のソフト指示 + 感応な変更を含む PR でだけ `/security-review` を促す nudge）、git 運用規約（Git Flow / Conventional Commits）、create-issue skill。任意で Copilot PR 自動レビュー（自動アサイン / watch-pr / resolve-pr）と AGENTS.md 自動生成 |
...
hook（PR 前レビュー gate・effort nudge は既定休眠だが本体ロジックを検証。security-review-nudge は感応判定と発火/抑制を検証）と apply.mjs の冪等性・登録・登録解除は `tests/` のユニットテストで検証する（Node 標準の test runner のみ・依存なし）:
