// テンプレート自動追随（.setup-sync.json + setup-sync-check.mjs）のテスト。
//
// 観点:
...
function writeState(projectDir, obj) {
  ...
  writeFileSync(join(projectDir, ".claude", ".setup-sync.json"), JSON.stringify(obj, null, 2) + "\n", "utf8");
}
...
test("setup-github apply が状態ファイルへ版と pr-copilot フラグを記録する", () => {
  ...
  const state = JSON.parse(readFileSync(join(target, ".claude", ".setup-sync.json"), "utf8"));
...
test("setup-github と setup-unity が状態ファイルへ各自のキーをマージ（相手を消さない）", () => {
  ...
  let state = JSON.parse(readFileSync(join(target, ".claude", ".setup-sync.json"), "utf8"));
  ...
  state = JSON.parse(readFileSync(join(target, ".claude", ".setup-sync.json"), "utf8"));
...
test("hook: 壊れた状態ファイルは黙って無視する", () => {
  ...
  writeFileSync(join(target, ".claude", ".setup-sync.json"), "{ not json", "utf8");
  assert.equal(runSyncHook(target, "9.9.9"), "");
});
