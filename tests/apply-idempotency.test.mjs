...
test("初回適用 → 再実行で重複せず、review-config が温存される", () => {
  ...
  const sync1 = JSON.parse(readFileSync(join(target, ".claude", ".setup-sync.json"), "utf8"));
  ...
  const sync2 = JSON.parse(readFileSync(join(target, ".claude", ".setup-sync.json"), "utf8"));
