...
// セキュリティレビュー行は撒かない（security-review-nudge.mjs が感応 PR でだけ促す）。
const MARKS = ["**ブランチ**:", "**簡素化**:"];
...
test("初回適用 → 再実行で重複せず、review-config が温存される", () => {
  ...
  // セキュリティレビュー行は無条件では撒かない（条件付き nudge が肩代わり）。
  assert.ok(!md1.includes("**セキュリティレビュー**:"), "無条件のセキュリティレビュー行が撒かれている");
  ...
  // security-review-nudge は base・常時有効で PostToolUse に登録される。
  assert.equal(settings1.hooks.PostToolUse.length, 1);
  const pt1 = settings1.hooks.PostToolUse.flatMap((g) => (g.hooks ?? []).map((h) => h.command));
  assert.ok(pt1.some((c) => c.includes("security-review-nudge.mjs")), "security nudge 未登録");
  ...
  assert.equal(settings2.hooks.PostToolUse.length, 1, "再実行で PostToolUse が重複した");
...
test("旧版配備への再実行: 旧レビュー行が /simplify へ移行し、無条件セキュリティ行は削除される", () => {
  const target = tempDir("apply-test-");
  mkdirSync(join(target, ".claude"), { recursive: true });
  // 旧版 setup-github 適用済みの CLAUDE.md を再現（無条件セキュリティ行あり・後続節あり）
  const oldMd = [
    "# プロジェクト規約",
    "",
    "## 開発ワークフロー",
  ...
  // 無条件セキュリティレビュー行は削除される（条件付き nudge が肩代わり）。
  assert.ok(!md.includes("**セキュリティレビュー**:"), "無条件セキュリティ行が削除されていない");
  assert.ok(!md.includes("/security-review"), "CLAUDE.md に /security-review 指示が残っている");
  // プロジェクト固有の後続節は保たれる。
...
test("pr-copilot 配備済みはフラグ無し再実行でも自動継承される", () => {
  ...
  const settings = JSON.parse(readFileSync(join(target, ".claude", "settings.json"), "utf8"));
  // PostToolUse = security-review-nudge（base）+ after-pr-create（pr-copilot）の 2 件。
  assert.equal(settings.hooks.PostToolUse.length, 2);
  const cmds = settings.hooks.PostToolUse.flatMap((g) => (g.hooks ?? []).map((h) => h.command));
  assert.ok(cmds.some((c) => c.includes("security-review-nudge.mjs")), "security nudge 未登録");
  assert.ok(cmds.some((c) => c.includes("after-pr-create.mjs")), "after-pr-create 未登録");
});
